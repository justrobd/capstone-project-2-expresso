...
employeesRouter.param('employeeId', (req, res, next, employeeId) => {
	...
	db.get(sql, values, (error, employees) => {
		if(error) {
			next(error);
		} else if (employees) {
...
employeesRouter.put('/:employeeId', (req, res, next) => {
	const name = req.body.employees.name,
		  position = req.body.employees.position, 
		  wage = req.body.employees.wage,
		  isCurrentEmployee = req.body.employees.isCurrentEmployee === 0 ? 0 : 1;
	if(!name || !position || !wage) {
		return res.sendStatus(400);
	}
